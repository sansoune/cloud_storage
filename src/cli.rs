...
pub async fn execute_command(storage_manager: &StorageManager, command: Commands) -> Result<(), StorageError> {
    match command {
        Commands::Upload { file } => {
            println!("Uploading file: {:?}", file);
            let data = tokio::fs::read(&file).await?;
            let filename = file.file_name().and_then(|n| n.to_str()).unwrap_or("unknown").to_string();
            let metadata = storage_manager.upload_file(&filename, &data).await?;
        ...
        Commands::Download { file_id, file_name, output } => {
            ...
            // Get the file data
            let data = storage_manager.download_file(&id).await?;
        ...
        Commands::Delete { file_id, file_name } => {
            ...
            match storage_manager.delete_file(&id).await {
