...
impl StorageManager {
    ...
    pub async fn upload_file(&self, filename: &str, data: &[u8]) -> Result<cloud_storage::FileMetadata> {
    ...
    pub async fn list_files(&self) -> Result<Vec<cloud_storage::FileMetadata>> {
        let storage = self.inner.lock().await;
